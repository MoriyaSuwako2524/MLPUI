...
独立评估已有模型：在“新建任务”将任务类型设为“评估已有模型”，填写模型文件绝对路径和相匹配的模型结构配置，再选择 npy 数据目录或分组。勾选能量、力评估项并选择 GPU 后开始。无需设置 epochs 或学习率；评估只遍历数据一次，不更新权重。详情页显示 MAE、RMSE、MSE，完整结果保存在任务目录的 `evaluation.json`，页面提供 JSON 链接。力指标按所有原子的笛卡尔分量汇总，单位沿用数据换算设置。评估与训练共用单任务运行限制。
