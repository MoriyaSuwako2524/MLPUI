...
训练支持可选早停：勾选“启用早停”，提供独立验证集，设置监控指标、Patience（默认 10）和 Min delta（默认 0）。默认监控验证集加权总损失，也可选择启用的能量、力或电荷 MSE。指标必须比上次有效改善的值降低超过 Min delta 才重置计数；连续 Patience 轮未达到这一条件则结束训练。测试集不参与决策。

启用早停时，`best.pt` 保存实际最低验证指标的权重，即使改善幅度小于 Min delta 也会更新；`model.pt` 保存训练结束时权重，不自动回滚。页面显示最佳 epoch、指标与停止原因，并提供最佳模型下载。早停结束后队列自动接续下一任务。缺少验证集时不能启用；加载旧 checkpoint 微调时重新统计早停，不恢复优化器或耐心计数。
